...
def register():
    ...
    try:
        ...
        if register_data.remember_me:
            ...
            session_duration = current_app.config.get('JWT_SESSION_REFRESH_TOKEN_EXPIRES', timedelta(hours=4))
        ...
        logger.info(f"Registration successful for user: {user.email} (remember_me: {register_data.remember_me})")
        return response
...
def login():
    ...
    try:
        ...
        if login_data.remember_me:
            ...
            session_refresh_expires = current_app.config.get('JWT_SESSION_REFRESH_TOKEN_EXPIRES', timedelta(hours=4))
            ...
            if current_app.config.get('JWT_COOKIE_CSRF_PROTECT', False):
                ...
                response.set_cookie(
                    ...
                    max_age=int(session_refresh_expires.total_seconds()),  # Match refresh token
                    secure=current_app.config.get('JWT_COOKIE_SECURE', False),
                    samesite=current_app.config.get('JWT_COOKIE_SAMESITE', 'Lax'),
                    path=current_app.config.get('JWT_REFRESH_CSRF_COOKIE_PATH', '/')
                )
        
        logger.info(f"Login successful for user: {user.email} (remember_me: {login_data.remember_me})")
        return response
