...
def setup_jwt_handlers(jwt):
    ...
    def check_if_token_revoked(jwt_header, jwt_payload):
        ...
        try:
            ...
            token_repo = UserTokenRepository(db_session)
            jti = jwt_payload['jti']
            
            is_revoked = token_repo.is_token_revoked(jti)
            if is_revoked:
    ...
    try:
        ...
        with app.app_context():
            ...
            if not has_tables:
                ...
                init_db()
                logger.info("Initial tables created (run 'python setup.py' for full setup)")
                ...
                logger.warning("Run 'python setup.py' to set up proper migration tracking")
                ...
                # Edge case
                logger.info("Database state unclear - tables exist but migration status unknown")
        ...
        env = app.config.get('ENV', 'development')  
        if env != 'production':
            logger.warning("Continuing despite database setup failure (development mode)")
            return False
        else:
...
def create_full_app():
    ...
    if env == 'development':
        ...
        logger.info("   Create migration: alembic revision --autogenerate -m 'description'")
