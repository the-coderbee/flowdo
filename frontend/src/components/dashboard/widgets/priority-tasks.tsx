...
import { usePriorityTasks } from "@/contexts/dashboard-context"
...
export function PriorityTasksWidget() {
  ...
  if (!priorityTasks || (priorityTasks.urgent.length === 0 && priorityTasks.high.length === 0)) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {priorityTasks.totalCount} high priority task{priorityTasks.totalCount !== 1 ? 's' : ''}
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          {/* Urgent Tasks */}
          {priorityTasks.urgent.length > 0 && (
          ...
          {/* View All Button */}
          {priorityTasks.totalCount > (priorityTasks.urgent.length + priorityTasks.high.length) && (
            <div className="pt-2 border-t">
              <Button variant="outline" size="sm" asChild>
                <Link href="/tasks?priority=urgent,high">
                  <ExternalLink className="h-4 w-4 mr-2" />
                  View All ({priorityTasks.totalCount})
                </Link>
              </Button>
            </div>
          )}
